...
const DryFruits = () => {
  ...
  useEffect(() => {
    const fetchBackendProducts = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5000/api/products?category=dryfruits');
        if (response.ok) {
  ...
  const baseProducts = backendProducts.length > 0 ? backendProducts : dryFruitsProducts;
